...
function addCriticalRole($pdo, $data) {
    try {
        ...
        return ['success' => true, 'message' => 'Critical role added successfully', 'id' => $pdo->lastInsertId()];
...
function getAvailableEmployees($pdo, $department = null) {
    try {
        if ($department) {
            $stmt = $pdo->prepare("SELECT id, full_name, department, position FROM users WHERE department = ? AND status = 'Active' ORDER BY full_name");
            $stmt->execute([$department]);
        } else {
            $stmt = $pdo->query("SELECT id, full_name, department, position FROM users WHERE status = 'Active' ORDER BY full_name");
        }
...
$highPotentials = $pdo->query("
    ...
    ORDER BY hpe.potential_score DESC
")->fetchAll();
                    ...
                    <div class="border border-gray-200 rounded-lg p-6">
                        ...
                        <div class="flex items-center justify-between mt-4">
                            ...
                            <button 
                                ...
                                class="ml-4 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors text-sm"
                            >
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                <?php foreach ($forecasts as $forecast): ?>
                    <div class="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow forecast-card" data-year="<?php echo $forecast['year']; ?>">
                        <p class="text-gray-600 mb-2"><?php echo $forecast['year']; ?></p>
                        <h3 class="text-2xl font-bold text-gray-900 mb-1 forecast-number">0</h3>
