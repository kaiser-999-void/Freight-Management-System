...
$stats = [
    ...
    'pending_certifications' => $pdo->query("SELECT COUNT(*) as count FROM skill_assessments WHERE status = 'Scheduled'")->fetch()['count'],
];

// Get upcoming retirements
$retirements = $pdo->query("
    SELECT cr.*, u.full_name as current_holder_name,
           GROUP_CONCAT(su.full_name SEPARATOR ', ') as successors
    FROM critical_roles cr
    LEFT JOIN users u ON cr.current_holder_id = u.id
    LEFT JOIN successors s ON cr.id = s.critical_role_id
    LEFT JOIN users su ON s.employee_id = su.id
    WHERE cr.retirement_date >= CURDATE()
    GROUP BY cr.id
...
<div class="p-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white rounded-lg shadow p-6 hover:shadow-lg transition-shadow cursor-pointer stat-card" data-stat="<?php echo $stats['pending_certifications']; ?>">
            <div class="flex items-start justify-between mb-4">
                <div class="bg-orange-500 p-3 rounded-lg">
                    <i class="fas fa-award text-white text-xl"></i>
                </div>
                <span class="text-red-600 text-sm">-3</span>
            </div>
            <h3 class="text-2xl font-bold text-gray-900 mb-1 stat-number">0</h3>
            <p class="text-gray-600">Pending Certifications</p>
        </div>
    </div>
        ...
        <div class="bg-white rounded-lg shadow">
            <div class="p-6 border-b border-gray-200">
                <div class="flex items-center gap-2">
                    ...
                    <h2 class="text-xl font-bold text-gray-900">Upcoming Retirements</h2>
                </div>
            </div>
            <div class="p-6">
                <div class="space-y-4">
                    <?php if (empty($retirements)): ?>
                        <p class="text-gray-500">No upcoming retirements</p>
                    <?php else: ?>
                        <?php foreach ($retirements as $retirement): ?>
                            <div class="border-l-4 border-orange-500 pl-4 py-2 hover:bg-orange-50 rounded-r-lg transition-colors cursor-pointer retirement-item" 
                                 onclick="showRetirementDetails(<?php echo htmlspecialchars(json_encode([
                                     'name' => $retirement['current_holder_name'] ?? 'TBD',
                                     'title' => $retirement['title'],
                                     'date' => $retirement['retirement_date'],
                                     'successors' => $retirement['successors'] ?? 'To be identified'
                                 ])); ?>)">
                                <div class="flex items-start justify-between mb-2">
                                    <div>
                                        <p class="text-gray-900 font-semibold"><?php echo htmlspecialchars($retirement['current_holder_name'] ?? 'TBD'); ?></p>
                                        <p class="text-gray-600"><?php echo htmlspecialchars($retirement['title']); ?></p>
                                    </div>
                                    <span class="text-gray-500 text-sm">
                                        <?php echo formatDate($retirement['retirement_date']); ?>
                                    </span>
                                </div>
                                <div class="flex items-center gap-2 text-sm">
                                    <i class="fas fa-users text-gray-400"></i>
                                    <span class="text-gray-600">
                                        Successor: <?php echo htmlspecialchars($retirement['successors'] ?? 'To be identified'); ?>
                                    </span>
                                </div>
                            </div>
                        <?php endforeach; ?>
    ...
    <div class="mt-8 bg-white rounded-lg shadow p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <a href="succession-planning.php" class="flex items-center gap-3 p-4 border-2 border-dashed border-gray-300 rounded-lg hover:border-indigo-500 hover:bg-indigo-50 transition-colors">
            ...
            <a href="training-management.php" class="flex items-center gap-3 p-4 border-2 border-dashed border-gray-300 rounded-lg hover:border-indigo-500 hover:bg-indigo-50 transition-colors">
                ...
                <span class="text-gray-700">Schedule Training</span>
            </a>
            ...
            <a href="competency-management.php?tab=promotion" class="flex items-center gap-3 p-4 border-2 border-dashed border-gray-300 rounded-lg hover:border-indigo-500 hover:bg-indigo-50 transition-colors">
...
<div id="trainingModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center">
    <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4">
        ...
        <div class="p-6 border-t border-gray-200 flex justify-end">
            <button onclick="closeTrainingModal()" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors mr-2">
                View Full Details
            </button>
